...
対応ゲームバージョン：**<!-- TARGET_GAME_VERSION_START -->1.15.19<!-- TARGET_GAME_VERSION_END -->-1.15.23**
...
SteamでStormworksを購入している場合は、購入済みのSteamアカウントでログインしてください。

1. 以下のリンクから、Steamワークショップにアクセスします。
...
4. `japanese.tsv`をStormworksの言語データディレクトリに移動させます。
   ...
   - 上記のパスはWindows環境でのデフォルトパスです。
     Windows以外のOSを使用している場合や、環境によっては上記のパスとは場所が異なる場合があります。
   - `AppData`は隠しフォルダです。
...
- 翻訳データを適用しても、英語のままであるテキストがあります。
  これらのテキストは、翻訳データを適用しても翻訳されません。
  あらかじめご了承ください。
- 誤字脱字の報告や翻訳に関する提案がありましたら、Steamワークショップのコメント欄または[Issues](https://github.com/Gakuto1112/Stormworks-JapaneseTranslation/issues)から、気軽にご報告・ご提案ください。
